...
- 保留模板的逻辑尺寸、root shell 形状、内部区域预算、核心层级、ID 命名和 `colorPolicy` 角色约束；输出时保持 `createSurface.width/height` 与 root `styles.width/height` 为目标数值尺寸，删除槽位时同步删除组件引用、DataModel 字段和 CardSpec 冗余。
