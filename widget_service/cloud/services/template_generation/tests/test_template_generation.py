...
def test_activity_daily_summary_stacks_supporting_metrics():
    ...
    supporting_metrics = next(
        node
        for node in _template_nodes(root, "Column")
        if len(node.children) == 2
        and all(child.component == "Row" for child in node.children)
    )
...
def test_pr7_visual_fixes_are_encoded_in_provider_cardtpl_variants():
    ...
    hero_content_options = _template_node_options(battery_hero.children[0])
    assert hero_content_options["layoutWeight"] == 1
    assert hero_content_options["justifyContent"] == "center"
    assert hero_content_options["alignItems"] == "center"
